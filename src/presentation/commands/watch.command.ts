...
import { AddWatcherUseCase } from '../../application/use-cases/add-watcher.use-case';
...
export const watchCommand = {
  data: new SlashCommandBuilder()
    .setName('watch')
    .setDescription("Watch a channel for '🟨Closed' messages")
    .addChannelOption((opt) =>
      opt
        .setName('channel')
        .setDescription('Channel to watch (optional)')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const useCase = new AddWatcherUseCase(new DynamoWatcherRepository(docClient));
    ...
    try {
      ...
      await interaction.editReply({ content: `✅ Now watching <#${target.id}>.` });
    } catch (err: unknown) {
      ...
      if (err instanceof UserError) {
        ...
        logger.error('watchCommand failed', err instanceof Error ? err : new Error(String(err)));
